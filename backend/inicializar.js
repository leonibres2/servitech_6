...
async function crearConfiguracionesSistema() {
  ...
  for (const config of configuraciones) {
    try {
      ...
      console.error(`❌ Error creando configuración ${config.clave}:`, error.message);
    }
  }
  
  console.log(`✅ ${contador} configuraciones del sistema creadas/actualizadas`);
}
...
async function crearUsuarioAdmin() {
  ...
  try {
    ...
    const adminExistente = await Usuario.findOne({ email: 'admin@servitech.com' });
    if (adminExistente) {
    ...
    await admin.save();
    console.log('✅ Usuario administrador creado (admin@servitech.com / admin123*)');
