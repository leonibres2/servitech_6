...
class CalendarioAsesorias {
        ...
        for (let semana = 0; semana < 6; semana++) {
            ...
            for (let dia = 0; dia < 7; dia++) {
                ...
                if (semana === 0 && dia < diaSemana) {
                    ...
                    if (this.fechaSeleccionada && this.esMismaFecha(fechaDia, this.fechaSeleccionada)) {
                    ...
                    if (fechaDia < hoy.setHours(0, 0, 0, 0)) {
                        ...
                        span.addEventListener('click', () => this.seleccionarFecha(fechaDia));
    ...
    mostrarHorarios(fecha) {
        ...
        const horarios = this.horariosDisponibles[fechaKey] || this.generarHorariosDefault();
        ...
        if (slotsContainer) {
            ...
            horarios.forEach(horario => {
                ...
                if (horario.ocupado) {
                    ...
                    slot.addEventListener('click', () => this.seleccionarHorario(horario.hora, slot));
    ...
    confirmarCita(event) {
        ...
        if (!this.fechaSeleccionada || !this.horaSeleccionada) {
            this.mostrarMensaje('Por favor selecciona una fecha y hora para tu cita.', 'warning');
            return;
        }

        // Crear objeto con datos de la cita
        const datosGita = {
            fecha: this.formatearFechaKey(this.fechaSeleccionada),
            hora: this.horaSeleccionada,
            experto: this.expertoActual.nombre,
            servicio: this.expertoActual.especialidad,
            duracion: '60 minutos'
        };

        // Guardar en localStorage para la página de pagos
        localStorage.setItem('citaSeleccionada', JSON.stringify(datosGita));

        // Mostrar mensaje de confirmación
        this.mostrarMensaje('Cita seleccionada correctamente. Redirigiendo a la pasarela de pagos...', 'success');
    ...
    mostrarMensaje(mensaje, tipo = 'info') {
        ...
        notificacion.className = `alert alert-${tipo === 'warning' ? 'warning' : tipo === 'success' ? 'success' : 'info'} alert-dismissible fade show`;
        ...
        notificacion.innerHTML = `
            <strong>${tipo === 'warning' ? '⚠️' : tipo === 'success' ? '✅' : 'ℹ️'}</strong> ${mensaje}
