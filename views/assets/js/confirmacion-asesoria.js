...
document.addEventListener('DOMContentLoaded', function() {
  // Inicializar página de confirmación con datos reales
  ...
  function inicializarConfirmacion() {
    ...
    if (!pagoCompleto) {
      mostrarError('No se encontraron datos del pago. Redirigiendo al inicio...');
      setTimeout(() => {
    ...
    try {
      ...
      // Mantener solo la información esencial para futuras referencias
      ...
      mostrarError('Error al cargar los datos de confirmación.');
  ...
  function actualizarInformacionPago(datos) {
    ...
    const metodoPagoContainer = document.querySelector('.payment-method-info-container');
  ...
  function usarDatosFallback() {
    ...
    if (fechaElement) {
      fechaElement.textContent = getRandomFutureDate();
    }
    
    if (horaElement) {
      horaElement.textContent = getRandomTimeSlot();
    }
    
    if (transactionElement) {
      transactionElement.textContent = generateTransactionId();
    }
  }
  ...
  function getRandomTimeSlot() {
    const hours = ['9:00 - 10:00', '10:30 - 11:30', '13:00 - 14:00', 
